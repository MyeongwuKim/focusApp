...
- 날짜별 할일 완료율과 집중/휴식 시간을 통계 화면에서 확인하는 구조
...
### 5) 알림 카테고리 분리 설계 (로컬 알림 / 서버 푸시 / 인앱 팝업)

- 처음에는 알림을 한 방식으로 묶어보려 했는데, 막상 운영해보니 성격이 달라서 같은 채널로 처리하면 불편함이 컸음
- 카테고리별 분리:
  - **로컬 알림(디바이스 예약)**: 휴식 종료, 목표 집중시간 도달, 할일 시작시간 도달
  - **서버 푸시(배치 발송)**: 오늘 할일 비어 있음(`empty_todo_start`), 미완료/중지 상태 리마인드(`incomplete_todo`)
- 왜 이렇게 나눴는지:
  - 로컬 알림은 방금 시작한 타이머/시각 이벤트라, 서버 주기보다 기기에서 바로 맞춰주는 게 정확함
  - 서버 푸시는 요일·시간대·인터벌·톤 같은 정책이 같이 움직여야 해서 배치에서 관리하는 쪽이 안정적이었음
- 로컬 알림 처리:
  - WebView에서 `REST_NOTIFICATION_SCHEDULE/CANCEL` 브릿지 메시지로 네이티브 예약 요청
  - 모바일에서 key 기반으로 예약 ID를 관리해서 중복 예약은 정리하고 필요한 건 다시 예약
  - 알림 데이터에 `targetPath`를 담아 탭 시 해당 날짜 할일 화면으로 바로 복귀
- 오늘 할일 화면에서의 처리:
  - 앱이 `isViewingTodayTodoSurface` 상태를 웹과 동기화
  - 이미 오늘 할일 화면을 보고 있으면 `startTodoPrompt`는 시스템 배너 대신 인앱 팝업으로 처리
  - 포그라운드에서 바로 `시작/오늘은 그만` 액션을 고를 수 있게 구성

### 6) 푸시 알림 안정화 (2026.05)

- 인터벌 변경 시 `pending interval` 반영과 다음 리마인드 계산 흐름 정리
- 재앵커링 조건, 발송 쿨다운, 락 처리 보강으로 중복 발송 구간 완화
- 시간대 변경 직후 미수신 케이스 보정, 푸시 토큰 등록 재시도 흐름 보강
