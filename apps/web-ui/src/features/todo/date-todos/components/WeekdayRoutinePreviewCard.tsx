    ...
    <div className="w-full max-w-sm space-y-3">
      <div className="space-y-1 text-center">
        <p className="m-0 pb-0.5 text-base font-semibold leading-snug text-base-content/85">
          적용된 요일 루틴이 있어요. 그대로 불러올까요?
        </p>
        <p className="m-0 text-xs text-base-content/60">아래 항목은 오늘 불러올 루틴 미리보기입니다.</p>
      </div>
      <div className="rounded-lg border border-base-300/70 bg-base-200/45 p-2.5">
        ...
        <div className="mt-2 space-y-1.5 opacity-70">
          {previewItems.map((item) => (
            <div
              ...
              className="truncate rounded-md border border-base-300/65 bg-base-100 px-2 py-1 text-xs text-base-content/70"
            >
              {item.content}
            </div>
          ))}
