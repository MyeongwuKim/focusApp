...
function TimeTooltip({ active, payload, label, showRest = true }: any) {
  if (!active || !payload || payload.length === 0) {
    return null;
  }
  const entry = payload[0]?.payload ?? {};
  const totalMinutes = showRest
    ? (entry.focusMin ?? 0) + (entry.deviationMin ?? 0) + (entry.restMin ?? 0)
    : (entry.focusMin ?? 0) + (entry.deviationMin ?? 0);
  ...
  return (
    <div className="rounded-lg border border-base-300 bg-base-100 px-3 py-2 text-xs shadow-lg">
      ...
      <p className="mt-1 text-base-content/70">총합 {totalMinutes}분</p>
    </div>
  );
}
